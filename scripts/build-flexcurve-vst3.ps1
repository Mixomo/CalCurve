...
Write-Host "FlexCurve root: $Root" -ForegroundColor DarkCyan
...
Write-Host "Configuring FlexCurve..." -ForegroundColor Cyan
...
Write-Host "Building FlexCurve VST3 and smoke tests..." -ForegroundColor Cyan
Invoke-VsCommand ("cmake --build " + (Quote-Arg $BuildPath) +
    " --target FlexCurve_VST3 FlexCurveEngineTest FlexCurveVST3SmokeTest --config " + (Quote-Arg $Config))

$Vst3Bundle = Join-Path $BuildPath "FlexCurve_artefacts\$Config\VST3\FlexCurve.vst3"
$SmokeTest = Join-Path $BuildPath "FlexCurveVST3SmokeTest_artefacts\$Config\FlexCurveVST3SmokeTest.exe"
$EngineTest = Join-Path $BuildPath "FlexCurveEngineTest_artefacts\$Config\FlexCurveEngineTest.exe"
...
if (-not $SkipTests) {
    if ($TestCurve.Trim().Length -gt 0) {
        ...
        Write-Host "Running engine test..." -ForegroundColor Cyan
        Invoke-TestCommand ((Quote-Arg $EngineTest) + " " + (Quote-Arg $TestCurve)) "FlexCurveEngineTest"
    } else {
        Write-Host "Skipping engine test: pass -TestCurve <file> to run it." -ForegroundColor Yellow
    }

    Write-Host "Running VST3 editor smoke test..." -ForegroundColor Cyan
    Invoke-TestCommand ((Quote-Arg $SmokeTest) + " " + (Quote-Arg $Vst3Bundle) + " --editor") "FlexCurveVST3SmokeTest --editor"

    Write-Host "Running VST3 preset roundtrip smoke test..." -ForegroundColor Cyan
    Invoke-TestCommand ((Quote-Arg $SmokeTest) + " " + (Quote-Arg $Vst3Bundle) + " --preset-roundtrip") "FlexCurveVST3SmokeTest --preset-roundtrip"
}

if (-not $NoBundleCopy) {
    $ReadyDir = Join-Path $Root "FlexCurve_VST3"
    $ReadyBundle = Join-Path $ReadyDir "FlexCurve.vst3"
...
Write-Host "FlexCurve build completed successfully." -ForegroundColor Green
